// src/pages/Dashboard.tsx
import { useEffect, useState } from "react";
import apiClient from "../config/apiClient";
import {
  ChevronRight,
  ChevronDown,
  AlertTriangle,
  CheckCircle2,
  XCircle,
} from "lucide-react";

/* ---------------------------------------------
   TYPES
----------------------------------------------*/

type FindingStatus = "GOOD" | "FAIR" | "FAIL" | string;

interface WeeklyFinding {
  status: FindingStatus;
  message: string;
}

interface MeetingRow {
  id: number;
  repName: string;
  customerName: string;
  primaryContact: string;
  meetingPurpose: string;
  meetingOutcome: string;
  createdAt?: string;
}

interface MeetingFinding {
  meetingId: number;
  status: FindingStatus;
  message: string;
}

interface KPIWeeklySnapshot {
  totalMeetings: number;
  score: number;
  status: FindingStatus;
  weeklyFindings?: WeeklyFinding[];
  counts?: {
    missingOutcomeCount?: number;
    missingContactCount?: number;
    roleOnlyCount?: number;
  };
  meetingFindings?: MeetingFinding[];
}

/* ---------------------------------------------
   CONFIG / REPS
----------------------------------------------*/

const reps = ["Ben", "Faith", "John", "Sarah"];

/* ---------------------------------------------
   HELPERS: status -> styles & icons
----------------------------------------------*/

function normalizeStatus(s?: string) {
  if (!s) return "FAIR";
  const up = s.toUpperCase();
  if (up === "FAIL") return "FAIL";
  if (up === "POOR") return "FAIL";
  if (up === "GOOD") return "GOOD";
  if (up === "FAIR") return "FAIR";
  return up;
}

function statusClasses(status?: FindingStatus) {
  const s = normalizeStatus(status as string);
  if (s === "GOOD")
    return {
      text: "text-emerald-700",
      bg: "bg-emerald-50",
      border: "border-emerald-200",
      accent: "emerald",
    };
  if (s === "FAIL")
    return {
      text: "text-rose-700",
      bg: "bg-rose-50",
      border: "border-rose-200",
      accent: "rose",
    };
  // FAIR default
  return {
    text: "text-amber-700",
    bg: "bg-amber-50",
    border: "border-amber-200",
    accent: "amber",
  };
}

function statusIcon(status?: FindingStatus) {
  const s = normalizeStatus(status as string);
  if (s === "GOOD")
    return <CheckCircle2 size={18} className="text-emerald-600" />;
  if (s === "FAIL") return <XCircle size={18} className="text-rose-600" />;
  // FAIR
  return <AlertTriangle size={18} className="text-amber-600" />;
}

/* ---------------------------------------------
   COMPONENT
----------------------------------------------*/

export default function Dashboard() {
  const [selectedRep, setSelectedRep] = useState<string>("Ben");
  const [kpi, setKpi] = useState<KPIWeeklySnapshot | null>(null);
  const [meetings, setMeetings] = useState<MeetingRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [openMeetingId, setOpenMeetingId] = useState<number | null>(null);

  useEffect(() => {
    if (!selectedRep) return;
    setLoading(true);

    Promise.all([
      apiClient.get(`/kpi/${selectedRep}`),
      apiClient.get(`/meetings/${selectedRep}`),
    ])
      .then(([kpiRes, meetingsRes]) => {
        const data = kpiRes.data ?? {};
        setKpi({
          totalMeetings:
            typeof data.totalMeetings === "number" ? data.totalMeetings : 0,
          score: typeof data.score === "number" ? data.score : 0,
          status: data.status ?? "FAIR",
          weeklyFindings: Array.isArray(data.weeklyFindings)
            ? data.weeklyFindings
            : [],
          counts: data.counts ?? {},
          meetingFindings: Array.isArray(data.meetingFindings)
            ? data.meetingFindings
            : [],
        });

        setMeetings(Array.isArray(meetingsRes.data) ? meetingsRes.data : []);
      })
      .catch((err) => {
        console.error("Dashboard fetch error:", err);
        setKpi(null);
        setMeetings([]);
      })
      .finally(() => setLoading(false));
  }, [selectedRep]);

  // Merge a meetingFinding with the full meeting row (if available)
  const getMeetingWithFinding = (mf: MeetingFinding) => {
    const meeting = meetings.find((m) => m.id === mf.meetingId) ?? null;
    return { finding: mf, meeting };
  };

  // Build the weekly findings text (trust backend status + totalMeetings)
  function weeklyFindingsMessage() {
    const status = normalizeStatus(kpi?.status);
    const total = kpi?.totalMeetings ?? 0;
    if (status === "GOOD") {
      return `Weekly Activity meets required minimum (5). Logged ${total} meetings.`;
    }
    if (status === "FAIL") {
      return `Weekly Activity critical. Logged ${total} meetings.`;
    }
    // FAIR
    return `Weekly Activity below required minimum (5). Logged ${total} meetings.`;
  }

  // convenience classes for weekly score card border highlight based on status
  const scoreCardBorder =
    normalizeStatus(kpi?.status) === "GOOD"
      ? "border-emerald-200"
      : normalizeStatus(kpi?.status) === "FAIL"
      ? "border-rose-200"
      : "border-amber-200";

  // make page scrollable - ensure wrapper has min-h and overflow
  return (
    <div className="min-h-screen pb-24 overflow-y-auto px-6 md:px-10 lg:px-12">
      {/* Header + rep chooser */}
      <div className="flex items-start justify-between mt-6 mb-6">
        <div>
          <h1 className="text-3xl font-extrabold">KPI Dashboard</h1>
          <p className="text-sm text-gray-500 mt-1">
            Weekly snapshot — shows findings and quality metrics for the
            selected rep.
          </p>
        </div>

        <div className="ml-4">
          <label className="sr-only" htmlFor="repSelect">
            Select rep
          </label>
          <select
            id="repSelect"
            value={selectedRep}
            onChange={(e) => setSelectedRep(e.target.value)}
            className="border rounded-md px-3 py-2 bg-white shadow-sm text-sm"
          >
            {reps.map((r) => (
              <option key={r} value={r}>
                {r}
              </option>
            ))}
          </select>
        </div>
      </div>

      {/* SUMMARY CARDS */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {/* Total meetings card */}
        <div className="bg-white rounded-xl shadow p-6">
          <p className="text-sm text-gray-500 font-medium">TOTAL MEETINGS</p>

          <div className="flex items-center justify-between mt-3">
            <div>
              <p className="text-4xl font-bold">{kpi?.totalMeetings ?? 0}</p>
              <p className="text-xs text-gray-400 mt-1">This week</p>
            </div>

            <div className="text-center">
              <div className="inline-block rounded-full bg-emerald-50 px-4 py-2 border border-emerald-100">
                <div className="text-xs text-emerald-700 font-semibold">
                  Score
                </div>
                <div className="text-xl font-bold">{kpi?.score ?? 0}</div>
              </div>
            </div>
          </div>

          <p className="text-xs text-gray-500 mt-4">
            Current productivity score based on meeting volume.
          </p>
        </div>

        {/* Weekly Score Card */}
        <div
          className={`bg-white rounded-xl shadow p-6 border ${scoreCardBorder} relative overflow-hidden`}
        >
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm text-gray-500 font-medium">
                WEEKLY SCORE CARD
              </p>
              <div className="mt-2 flex items-end gap-4">
                <p className="text-4xl font-extrabold">{kpi?.score ?? 0}</p>
                <p className="text-sm text-gray-500 mt-1">/ 100</p>
              </div>
            </div>

            <div>
              <div
                className={`inline-flex items-center px-3 py-1 rounded-md text-sm font-semibold ${
                  statusClasses(kpi?.status).text
                } ${statusClasses(kpi?.status).bg}`}
              >
                {kpi?.status ?? "FAIR"}
              </div>
            </div>
          </div>

          <div className="mt-4">
            <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
              <div
                className={`h-3 rounded-full transition-all duration-700 ${
                  (kpi?.score ?? 0) >= 70
                    ? "bg-emerald-500"
                    : (kpi?.score ?? 0) >= 45
                    ? "bg-amber-500"
                    : "bg-rose-500"
                }`}
                style={{
                  width: `${Math.max(0, Math.min(100, kpi?.score ?? 0))}%`,
                }}
              />
            </div>

            <div className="mt-3 text-xs text-gray-500">
              Target is &gt; 70.{" "}
              {(kpi?.score ?? 0) >= 70
                ? "Excellent performance this week."
                : (kpi?.score ?? 0) >= 45
                ? "Needs improvement in meeting frequency."
                : "Score is below target (> 70)."}
            </div>
          </div>
        </div>

        {/* Weekly Findings */}
        <div className="bg-white rounded-xl shadow p-6">
          <p className="text-sm text-gray-500 font-medium">WEEKLY FINDINGS</p>

          <div className="mt-4">
            {/* Use backend status and total for dynamic message */}
            <div className="flex items-start gap-3">
              <div
                className={`p-2 rounded-md ${statusClasses(kpi?.status).bg} ${
                  statusClasses(kpi?.status).border
                } ${statusClasses(kpi?.status).text} border`}
              >
                {statusIcon(kpi?.status)}
              </div>

              <div>
                <div className="font-semibold text-sm">
                  {normalizeStatus(kpi?.status) === "GOOD"
                    ? "Good Activity"
                    : normalizeStatus(kpi?.status) === "FAIL"
                    ? "Poor Activity"
                    : "Fair Activity"}
                </div>
                <div className="text-sm text-gray-500 mt-1">
                  {weeklyFindingsMessage()}
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* Meeting Findings list */}
      <div className="space-y-6 mb-20">
        <div className="flex items-center gap-3">
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
            fill="none"
            className="text-sky-600"
          >
            <path
              d="M3 7h18M3 12h18M3 17h18"
              stroke="#0ea5e9"
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </svg>
          <h2 className="text-xl font-semibold">Meetings Breakdown</h2>
        </div>

        {loading && <div className="text-gray-500">Loading…</div>}

        {!loading && (kpi?.meetingFindings?.length ?? 0) === 0 && (
          <div className="bg-white rounded-xl p-6 shadow text-center text-gray-500">
            No meeting findings for this week.
          </div>
        )}

        <div className="space-y-4">
          {!loading &&
            (kpi?.meetingFindings ?? []).map((mf) => {
              const { meeting } = getMeetingWithFinding(mf);
              const isOpen = openMeetingId === mf.meetingId;
              const sc = statusClasses(mf.status);

              return (
                <div
                  key={mf.meetingId}
                  className="bg-white rounded-xl shadow overflow-hidden border"
                >
                  {/* Header */}
                  <div
                    className={`flex items-center justify-between px-6 py-4 border-b ${
                      isOpen ? "" : ""
                    }`}
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center border ${sc.border} ${sc.bg} transition-colors`}
                      >
                        <div className={`text-sm font-semibold ${sc.text}`}>
                          {normalizeStatus(mf.status) === "GOOD"
                            ? "✓"
                            : normalizeStatus(mf.status) === "FAIL"
                            ? "✕"
                            : "!"}
                        </div>
                      </div>

                      <div>
                        <p className="text-lg font-semibold">
                          Meeting at{" "}
                          <span className="font-bold">
                            {meeting?.customerName ?? `#${mf.meetingId}`}
                          </span>
                        </p>
                        <p className="text-xs text-gray-400">{mf.status}</p>
                      </div>
                    </div>

                    <div>
                      <button
                        onClick={() =>
                          setOpenMeetingId(isOpen ? null : mf.meetingId)
                        }
                        className="text-sm text-gray-600 hover:text-gray-900 flex items-center gap-2 transition"
                        aria-expanded={isOpen}
                        aria-controls={`meeting-${mf.meetingId}`}
                      >
                        {isOpen ? (
                          <>
                            Close <ChevronDown size={16} className="ml-1" />
                          </>
                        ) : (
                          <>
                            View <ChevronRight size={16} className="ml-1" />
                          </>
                        )}
                      </button>
                    </div>
                  </div>

                  {/* Content */}
                  {isOpen && (
                    <div
                      id={`meeting-${mf.meetingId}`}
                      className="px-6 py-6 space-y-6 bg-gray-50"
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                          <p className="text-xs font-semibold text-gray-500">
                            PRIMARY CONTACT
                          </p>
                          <p className="mt-2 text-sm text-gray-800">
                            {meeting?.primaryContact ?? "—"}
                          </p>
                        </div>

                        <div>
                          <p className="text-xs font-semibold text-gray-500">
                            MEETING PURPOSE
                          </p>
                          <p className="mt-2 text-sm text-gray-800">
                            {meeting?.meetingPurpose ?? "—"}
                          </p>
                        </div>
                      </div>

                      <div className="bg-white rounded-lg border p-4">
                        <p className="text-xs font-semibold text-gray-500">
                          MEETING OUTCOME
                        </p>
                        <p className="text-sm mt-2 text-gray-800 leading-relaxed">
                          {meeting?.meetingOutcome ?? mf.message}
                        </p>
                      </div>

                      {/* Finding block styled according to finding status */}
                      <div
                        className={`${
                          normalizeStatus(mf.status) === "GOOD"
                            ? "bg-emerald-50 border-emerald-100 text-emerald-700"
                            : normalizeStatus(mf.status) === "FAIL"
                            ? "bg-rose-50 border-rose-100 text-rose-700"
                            : "bg-amber-50 border-amber-100 text-amber-700"
                        } border rounded-lg p-4`}
                      >
                        <div className="text-xs font-semibold">
                          {normalizeStatus(mf.status) === "GOOD"
                            ? "QUALITY CHECK"
                            : "FINDING"}
                        </div>
                        <div className="text-sm mt-2">{mf.message}</div>
                      </div>
                    </div>
                  )}
                </div>
              );
            })}
        </div>
      </div>
    </div>
  );
}
